...
class FactualityScorer(Scorer):
    """
    Use an LLM to check if the generated answer is factually correct according to the reference answer.
    """
    ...
    def validate_sample(cls, sample: Sample) -> bool:
        valid_sample = (
            ...
            and "reference_answer" in sample.evaluation.data
        )
        if not valid_sample:
    ...
    async def score(
        ...
        reference_answer = sample_with_outputs.sample.evaluation.data[
            "reference_answer"
        ]
